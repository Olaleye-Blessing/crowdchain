...
contract CrowdfundingUpdates is Crowdfunding, ICampaignUpdates {
    ...
    /// @inheritdoc ICampaignUpdates
    function getUpdate(uint256 campaignId, uint256 updateId) external view campaignExists(campaignId) returns (Update memory update) {
    ...
    function getCampaignUpdates(
        ...
        uint256 page,
        uint256 perPage
    ) external view campaignExists(campaignId) returns (Update[] memory updates, uint256 total) {
