...
contract CrowdfundingUpdatesTest is Test, ConstantsTest {
    ...
    function test_getUpdateFailsIfIdDoesNotExist() external {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(ICampaignUpdates.CampaignUpdates__UpdateNotExist.selector, 0)
      );
    ...
    function test_getCorrectNumberOfUpdates() external {
      ...
      for (uint256 index = 0; index < totalUpdatesToCreate; index++) {
        _createUpdate(ALICE, 0, string.concat(NUMBER_IN_WORDS[index], VALID_TITLE), string.concat(NUMBER_IN_WORDS[index], VALID_CONTENT));
      }

      uint256 page = 0;
      uint256 perPage = 2;

      (ICampaignUpdates.Update[] memory updates, uint256 totalUpdates) = crowdfundingUpdates.getCampaignUpdates(0, page, perPage);
    ...
    function _createCampaign(address _owner) private {
        ...
        crowdfundingUpdates.createCampaign(title, summary, description, coverImage, milestones, categories, amountNeeded, deadline, refundDeadline);
