...
export const wagmiAbi = parseAbi([
  ...
  "struct CampaignDetails { uint256 id; uint256 amountRaised; uint256 deadline; uint256 refundDeadline; uint256 goal; uint256 totalDonors; uint256 tokensAllocated; uint8 totalMilestones; uint8 currentMilestone; uint8 nextWithdrawableMilestone; address owner; string title; string description; string coverImage; bool claimed; string[] categories }",
  "struct Milestone { uint8 id; uint256 targetAmount; uint256 deadline; string description; uint8 status; }",

  // =============== FUNCTIONS ==============
  "function createCampaign(string title, string description, string coverImage, BasicMilestone[] milestones, string[] categories, uint256 goal, uint256 duration, uint256 refundDeadline) public",
