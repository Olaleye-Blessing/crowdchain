const campaignDetails = `tuple(uint256 id, uint256 amountRaised, uint256 deadline, uint256 refundDeadline, uint256 goal, uint256 totalDonors, uint256 tokensAllocated, uint8 totalMilestones, uint8 currentMilestone, uint8 nextWithdrawableMilestone, address owner, string title, string description, string coverImage, bool claimed)`;
...
export const crowdChainABI = [
  ...
  `function getOwnerCampaigns(address owner, uint256 page, uint256 perPage) external view returns (${campaignDetails}[], uint256)`,
