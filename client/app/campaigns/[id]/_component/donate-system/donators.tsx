      ...
      <FetchedData item={donors}>
        {donors.data && (
          <ul className="px-4 overflow-y-auto max-h-[20rem] flex flex-col space-y-2">
            {Object.entries(donors.data).map(([donor, amount]) => {
              return (
                <li key={donor} className="flex items-center justify-between">
                  <span title={donor} className="text-gray-500 font-semibold">
                    {formatAddress(donor)}
                  </span>
                  <span>{amount}</span>
                </li>
